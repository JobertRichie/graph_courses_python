...
```{python}
# Your code here
population_subset = population.query("indicator == 'SP.POP.TOTL'")
population_subset
```
...
hiv_data_combined_pop["incidence_rate"] = (
    hiv_data_combined_pop["new_cases"] * 100000 / hiv_data_combined_pop["population"]
)

hiv_data_combined_pop["prevalence_rate"] = (
    hiv_data_combined_pop["total_cases"] * 100000 / hiv_data_combined_pop["population"]
)
...
```{python}
# Your code here
hiv_data_mini = hiv_data_combined_pop.query("country in ['Cameroon', 'Benin', 'Malaysia', 'Mexico']")
...
```{python}
# Your code here
##Adding regional information for each country
hiv_data_combined_pop["continent"] = cc.convert(
    hiv_data_combined_pop["country"], to = "continent"
)
hiv_data_combined_pop
```

```{python}
# Highest prevalence rates per continent for 2011
hiv_data_combined_pop_2011 = hiv_data_combined_pop.query("year == '2011'")#subset for 2011

highest_prev_2011 = hiv_data_combined_pop_2011.loc[hiv_data_combined_pop_2011.groupby("continent")["prevalence_rate"].idxmax()]

highest_prev_2011
```

```{python}
# Bar chart comparing the prevalence rate estimates of these countries
px.histogram(
    highest_prev_2011,
    x = "country",
    y = "prevalence_rate",
