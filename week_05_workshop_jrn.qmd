...
```{python}
# Your code here
demog_loan_info = df_1[["emp_title", "emp_length", "state", "homeownership"]]
demog_loan_info
```
...
) # The term variable is not contained in the subset just created above
```
...
```{python}
# Your code here
demog_loan_info.to_csv('Demographic_Information.csv', index = False)
```
...
```{python}
# Your code here
credit_vars = df_2.filter(regex = "credit")
credit_vars
```
...
```{python}
# Your code here
high_income = df_2.query("annual_income > 100000")
high_income
...
```{python}
# Your code here
high_risk = df_2.query(
    "interest_rate > 15 | debt_to_income > 30 | delinq_2y == '2'")
high_risk
...
# Your code here
prime = df_2.query(
    "grade.isin(['A', 'B']) & debt_to_income < 20 & delinq_2y != '2'"
)
prime
